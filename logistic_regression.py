...
class LogisticRegression():
    ...
    def binary_cross_entropy_loss(self, y:np.ndarray, y_predict:np.ndarray)->np.ndarray:
        ...
        return np.sum(-((y * np.log(y_predict + epsilon)) + ((1 - y) * (np.log(1 - y_predict + epsilon))))) / len(y_predict)
    ...
    def sigmoid(self, z:np.ndarray)->np.ndarray:
        ...
        # print("max",np.max(z))
        # print("min",np.min(z))
        # set max and min z amount so doesn't overflow in sigmoid, this is for unnormalized data in predict
        return 1 / (1 + np.exp(-z))
    ...
    def fit(self, y:np.ndarray, X:np.ndarray)-> None:
        ...
        for i in range(self._iterations):
            y_predict:float = self.predict(X)
